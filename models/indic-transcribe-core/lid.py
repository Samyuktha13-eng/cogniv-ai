...
TRAINED_LANGS = (
    "as", "bhb", "bho", "bn", "brx", "doi", "en", "gu", "hi",
...
RECOMMENDED_LANGS = tuple(x for x in TRAINED_LANGS if x not in ("bhb",))
...
def language_token_map(tokenizer, allowed_langs: Sequence[str] | None = None) -> dict[int, str]:
    ...
    All 25 languages this checkpoint is trained on are present in its vocabulary, so no
    ...
    tokens (the inherited ISO-639 world list) while this checkpoint is trained on 25, so
